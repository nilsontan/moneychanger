...
import com.techstudio.erp.moneychanger.client.view.PosUiHandlers;
...
public class PosPresenter
  ...
  /**
   * {@link com.techstudio.erp.moneychanger.client.presenter.PosPresenter}'s proxy.
   */
  @ProxyCodeSplit
  ...
  private final MyDataProvider<PricingProxy> pricingDataProvider;
  ...
  public PosPresenter(final EventBus eventBus,
                      ...
                      final Provider<TransactionRequest> transactionRequestProvider) {
    super(eventBus, view, proxy);
    getView().setUiHandlers(this);
    getView().showLoading(true);
    this.itemRequestProvider = itemRequestProvider;
    this.categoryRequestProvider = categoryRequestProvider;
    this.transactionRequestProvider = transactionRequestProvider;
  ...
  public void saveTransaction() {
    ...
    if (transactionProxy == null) {
      transactionProxy = transactionRequest.create(TransactionProxy.class);
    }
    transactionProxy.setLineItems(new ArrayList<LineItemProxy>());
    transactionRequest.save(transactionProxy)
        .with("lineItems")
  ...
  private void saveItems() {
    ...
    transactionRequest = transactionRequestProvider.get();
    LineItemProxy proxy = transactionRequest.create(LineItemProxy.class);
    LineItemProxy pendingProxy = lineItems.remove(0);
    proxy.setTransactionType(pendingProxy.getTransactionType());
    proxy.setItemBuy(pendingProxy.getItemBuy());
    proxy.setCatBuy(pendingProxy.getCatBuy());
    proxy.setItemSell(pendingProxy.getItemSell());
    proxy.setCatSell(pendingProxy.getCatSell());
    proxy.setBuyQuantity(pendingProxy.getBuyQuantity());
    proxy.setSellQuantity(pendingProxy.getSellQuantity());
    proxy.setBuyUnitPrice(pendingProxy.getBuyQuantity());
    proxy.setSellUnitPrice(pendingProxy.getSellUnitPrice());

    transactionProxy = transactionRequest.edit(transactionProxy);
    transactionProxy.getLineItems().add(proxy);
    transactionRequest.save(transactionProxy)
        .with("lineItems")
        .fire(new Receiver<TransactionProxy>() {
          @Override
  ...
  private LineItemProxy createLineItem(ItemProxy itemToBuy, ItemProxy itemToSell) {
    ...
    proxy.setItemBuy(itemToBuy.getCode());
    ...
    proxy.setCatBuy(itemToBuy.getCategory().getCode());

    proxy.setItemSell(itemToSell.getCode());
    ...
    proxy.setCatSell(itemToSell.getCategory().getCode());
  ...
  private void updateItemDetailsView() {
    ItemProxy itemToBuy = itemDataProvider.getByCode(pendingLineItem.getItemBuy());
    ItemProxy itemToSell = itemDataProvider.getByCode(pendingLineItem.getItemSell());
  ...
  private CategoryProxy findItemBuyCategory() {
    ItemProxy itemBuy = itemDataProvider.getByCode(pendingLineItem.getItemBuy());
    return findCategory(itemBuy.getCategory().getCode());
  }

  private CategoryProxy findItemSellCategory() {
    ItemProxy itemSell = itemDataProvider.getByCode(pendingLineItem.getItemSell());
    return findCategory(itemSell.getCategory().getCode());
  }
